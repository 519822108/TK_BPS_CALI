...
CtlCan::CtlCan(int type,int index,int cindex,int port)
{
    devType = type;
    devIndex = index;
    canIndex = cindex;
...
{
    return !VCI_StartCAN(devType,devIndex,canIndex);
}
...
int CtlCan::rec_data_update()
{
    ...
    if(rec_len = 0xFFFFFFFF)    return 1;
 ...
 *          romete: whether is romete frame or si data frame,default is data frame
 *          type:   send mode,default is nomal send
 *      @return:    achieve return 0,otherwise return 1
*/
int CtlCan::send_one_msg(unsigned int ID, char len, char *data, isExtFram frame, isRomete romete, sendType type)
{
    VCI_CAN_OBJ obj;
    if(len >= 8 || len < 0)
        return 1;
